...
function Dock() {
  ...
  const playDockHover = () => {
    if (!soundEnabled || !canUseAudio) return;
    const now = Date.now();
    if (now - lastDockHoverAt.current < 90) return;
    lastDockHoverAt.current = now;
    playClick("hover");
  };
  ...
  return (
    ...
    >
      <AppIcon mouseX={mouseX} onHoverSound={playDockHover}>
        ...
        {pathname === "/" && <span className="pagetrack active"></span>}
      </AppIcon>
      ...
      <AppIcon mouseX={mouseX} onHoverSound={playDockHover}>
        ...
        {pathname === "/aboutme" && <span className="pagetrack active"></span>}
      </AppIcon>
      <AppIcon mouseX={mouseX} onHoverSound={playDockHover}>
        <Link
          href={"/projectspage"}
      ...
      <AppIcon mouseX={mouseX} onHoverSound={playDockHover}>
        <Link
          href={"mailto:hebrahimnori@gmail.com"}
          data-tooltip-id="footer-tooltip"
      ...
      <AppIcon mouseX={mouseX} onHoverSound={playDockHover}>
        ...
        >
          {theme === "dark" ? (
            ...
            >
              <path
                d="M12 3c-4.97 0-9 4.03-9 9s4.03 9 9 9a9 9 0 009-9c0-.39-.04-.77-.11-1.13a6.99 6.99 0 01-7.89-11.74A8.97 8.97 0 0012 3z"
                fill="#7A7A7A"
              />
            </svg>
          )}
      ...
      <AppIcon mouseX={mouseX} onHoverSound={playDockHover}>
        <button
          type="button"
          data-sound-toggle
          onClick={() => canUseAudio && toggleSound()}
          disabled={!canUseAudio}
          data-tooltip-id="footer-tooltip"
          data-tooltip-content={
            !canUseAudio
              ? "Sound on Hebi (light) — Parsa later"
              : soundEnabled
                ? "Mute ambience & clicks"
                : "Play light ambience & clicks"
          }
          data-tooltip-place="top"
          aria-label={
            !canUseAudio
              ? "Sound available in Hebi light theme"
              : soundEnabled
                ? "Turn off sound"
                : "Turn on sound"
          }
          aria-pressed={soundEnabled}
          style={{ opacity: canUseAudio ? 1 : 0.45 }}
        >
          <DockSoundIcon on={soundEnabled} disabled={!canUseAudio} />
        </button>
      </AppIcon>
      <Tooltip
        id="footer-tooltip"
...
function AppIcon({ mouseX, children, onHoverSound }: AppIconProps) {
